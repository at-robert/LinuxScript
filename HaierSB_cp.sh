...
cp $INPATH/H2921A/Application/H2921A.bin $OUTPATH/$VER/'HSD3A260_APP_'$VER.bin
md5 $OUTPATH/$VER/'HSD3A260_APP_'$VER.bin
sync

elif [ "$1" == "35" ]; then
echo "Haier 35 2.1 app"
cp $INPATH/H3521A/Application/H3521A.bin $OUTPATH/$VER/'HSD3A360_APP_'$VER.bin
md5 $OUTPATH/$VER/'HSD3A360_APP_'$VER.bin
sync

elif [ "$1" == "2920" ]; then
echo "Copy Haier 29 2.0 app"
cp $INPATH/H2921A/Application/H2921A.bin $OUTPATH/$VER/'HSD2A120_APP_'$VER.bin
md5 $OUTPATH/$VER/'HSD2A120_APP_'$VER.bin
sync

elif [ "$1" == "2920r" ]; then
echo "Haier 29 2.0 release "$VER
cp $INPATH/H2921A/Release/H2921A_SMT.bin $OUTPATH/$VER/'HSD2A120_Release_'$VER.bin
md5 $OUTPATH/$VER/'HSD2A120_Release_'$VER.bin
sync

elif [ "$1" == "29r" ]; then
echo "Haier 29 2.1 release "$VER
cp $INPATH/H2921A/Release/H2921A_SMT.bin $OUTPATH/$VER/'HSD3A260_Release_'$VER.bin
md5 $OUTPATH/$VER/'HSD3A260_Release_'$VER.bin
sync

elif [ "$1" == "35r" ]; then
echo "Haier 35 2.1 release"
cp $INPATH/H3521A/Release/H3521A_SMT.bin $OUTPATH/$VER/'HSD3A360_Release_'$VER.bin
md5 $OUTPATH/$VER/'HSD3A360_Release_'$VER.bin
sync
...
rar a -r -ep1 `date '+ %Y%m%d'`'_Haier2ndG_29_35_'$VER'_'$SVN.rar $OUTPATH/$VER
md5 `date '+ %Y%m%d'`'_Haier2ndG_29_35_'$VER'_'$SVN.rar
mv `date '+ %Y%m%d'`'_Haier2ndG_29_35_'$VER'_'$SVN.rar $OUTPATH
